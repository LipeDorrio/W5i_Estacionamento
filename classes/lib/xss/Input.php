<?php  
require_once($path_inc."/classes/lib/xss/Config.php");
/**
 * CodeIgniter 1.7.2
 *
...
*/
	...
	*/
	function CI_Input()
	{
		if ($this->debug) echo "<font color='#FF0000'>Construtor</font><br>";

		$CFG = new CI_Config();
		$this->use_xss_clean	= FALSE;//($CFG->item('global_xss_filtering') === TRUE) ? TRUE : FALSE;
		$this->allow_get_array	= TRUE;//($CFG->item('enable_query_strings') === TRUE) ? TRUE : FALSE;
		$this->_sanitize_globals();

		if ($this->debug) echo "<font color='#FF0000'>Fim Construtor</font><br>";
	}
	...
	* @return	string
	*/
	function xss_clean($str, $is_image = FALSE, $permiteHtml = FALSE)
	{
		if ($this->debug) echo "<font color='#FF0000'>Entrou XSS Clean</font><br>";
		/*
		...
		*/
		...
		//retira as tagas html se não permitir html
		if (!$permiteHtml){
			//$str = htmlentities(strip_tags($str));//ENT_QUOTES);
			$str = strip_tags($str);
		}
		if ($this->debug) echo "<font color='#FF0000'>Fim XSS Clean</font><br>";
