...
class VeiculoRepository
{
    ...
    {
        $query = "insert into veiculo (id_categoria, placa) 
                    values (" . $obj->idCategoria->getId() . ",'" . $obj->getPlaca() . "')";
    ...
    public function listAllConst($conn, $param)
    {
        $query = " select   veiculo.id, veiculo.id_categoria, veiculo.placa, categoria.id as id_idCategoria,
                    ...
                    where   veiculo.id_categoria = categoria.id ".(isset($param["comp"]) ? $param["comp"] : "");
        if (!$result = $conn->Execute($query))
            throw new Exception("[REPOSITORY]->" . $conn->ErrorMsg());
        return $result;
    }
